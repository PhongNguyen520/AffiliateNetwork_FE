...
const CampaignDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchCampaign = async () => {
      try {
        const response = await requestsPrivate.get(`${CAMPAIGN_DETAIL_URL}/${campaignId}`);
        if (!response.data) {
  ...
  const handleJoinCampaign = async () => {
    try {
      const response = await requestsPrivate.post(`${JOIN_CAMPAIGN_URL}/${campaignId}/join`);
  ...
  if (!campaign) return <div className="text-center p-5">No campaign data available</div>;
  ...
  return (
    <Container>
      <Row className="mb-4">
        ...
        <Col md={7}>
          ...
          <div className="d-flex align-items-center mt-2">
            ...
            <span>{formatDate(campaign.startDate)} - {campaign.endDate ? formatDate(campaign.endDate) : "Now"}</span>
          </div>
      ...
      <div className="mb-4">
        ...
        <p className="mt-3">
          From the passion of bringing quality products with the best value to consumers. {campaign.campaignName} was born with the desire
          to create a top shopping event, harmoniously connecting brands and customers, between needs and opportunities.
        </p>

        <div className="mt-4">
          <h5 className="fw-bold">Important information of the campaign:</h5>
          <p className="mt-3">
            Note: The orders that publishers have just clicked on will be orders generated on the system. AT will return the order status
            twice a week on every Monday and Thursday. (The average approval rate of the campaign is from 65-70% of the number of
            orders generated).
          </p>
          <p>
            In case customers have problems with the discount code or cannot complete the order = Please fill out the Incident Report
            form: Here, the AT team will respond within 24 hours to support you!
          </p>

          <h5 className="fw-bold mt-4">Target Customer:</h5>
          <p className="mt-2">{campaign.targetCustomer}</p> {/* Đối tượng khách hàng từ API */}
          <p>Primary focus on tech-savvy shoppers aged 25-45 with disposable income, interested in electronics, home goods, and
            fashion. Secondary focus on bargain hunters across all demographics.</p>

          <h5 className="fw-bold mt-4">***SPECIAL: {campaign.campaignName} SUPER OFFER</h5>
          <ul className="mt-2">
            <li>Pre-Black Friday (20/11-27/11): Commission increased by 1.5x for all successful orders</li>
            <li>Black Friday Official (28/11-29/11): Commission increased by 2x for all successful orders</li>
            <li>Cyber Monday (02/12): Commission increased by 1.8x for all successful orders</li>
            <li>Additional bonus of 5,000,000 VND for Top 5 Publishers with the highest sales in the entire campaign</li>
          </ul>

          <h5 className="fw-bold mt-4">***GUIDE TO OPTIMIZE CAMPAIGN PERFORMANCE</h5>
          <ul className="mt-2">
            <li>Focus on promoting during golden hours: 12:00-14:00 and 20:00-22:00 daily</li>
            <li>Prioritize using Deep Link to increase conversion rate</li>
            <li>Combine with price comparison content to increase credibility</li>
            <li>Use Social Media channels during the 19:00-21:00 time frame to reach the largest number of users</li>
          </ul>
      ...
      <div className="container mt-4">
        ...
        <div className="btn-group mb-3">
          {["All", 5, 4, 3, 2, 1].map((star, index) => (
            <Button
              key={index}
              variant={filter === star || (filter === "all" && star === "All") ? "warning" : "outline-warning"}
              onClick={() => setFilter(star === "All" ? "all" : star)}
            >
              {star}{typeof star === "number" ? "*" : ""}
            </Button>
          ))}
        </div>
        ...
        <Button variant="link" className="text-warning mt-3">See more</Button>
      </div>
    </Container>
  );
};
